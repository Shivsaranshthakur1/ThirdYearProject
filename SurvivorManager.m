...
classdef SurvivorManager < handle
    ...
    methods
        ...
        function survivors = getUntaggedSurvivors(obj)
            % Return survivors who are not yet 'DETECTED' or 'RESCUED'
            allSurvivors = obj.Survivors;
            isUntagged   = true(size(allSurvivors));
            for i = 1:length(allSurvivors)
                if strcmp(allSurvivors(i).Status, 'DETECTED') || ...
                   strcmp(allSurvivors(i).Status, 'RESCUED')
                    isUntagged(i) = false;
                end
            end
            survivors = allSurvivors(isUntagged);
        end
        ...
        function survivors = getUnvisitedSurvivors(obj)
            % Returns all survivors whose 'Visited' property is false
            allSurvivors = obj.Survivors;
            isUnvisited = arrayfun(@(s) ~s.Visited, allSurvivors);
            survivors   = allSurvivors(isUnvisited);
        end
        ...
        function reportDetection(obj, survivorID)
            try
                ...
                if strcmp(survivor.Status, 'UNDETECTED')
                    ...
                    survivor.Visited = true; % Mark as visited
